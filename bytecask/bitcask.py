...
# data file entry
DataEntry = namedtuple("DataEntry", ['crc32', 'tstamp', 'key_sz',
                                     'value_sz', 'key', 'value'])
...
class DataFile(object):
    ...
    def _read(self, fmmap, pos):
        ...
        if crc32_new == crc32:
            return DataEntry(crc32, tstamp, key_sz, value_sz,
                             key, value_sz), pos
        else:
...
class BitCask(object):
    ...
    def _find_data_files(self):
        ...
        if not self._active_file:
            self._active_file = self._build_new_Active_File()

    def _build_new_Active_File(self):
        """create new active file and open active file with `rb` and `ab`
        """
        ...
        return ActiveFile(self.base_path, "%d.%s" % (active_file_id, ACTIVE))
    ...
    def put(self, key, value):
        ...
        if self._should_make_immutable(key, value):
        ...
        if value != TOMBSTONE:
            self._keydir[key] = KeydirEntry(self._active_file.file_id, tstamp, value_pos, value_sz)
